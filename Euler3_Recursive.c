...
long long findTheLargestPrimeFactor(long long num, long long temp, long long i, long long limit, long long result)
{
	if(i<=limit)
	{
		if(temp%i==0)
		{
			limit=temp/i;
			if(isPrime(limit,2))
				findTheLargestPrimeFactor(num,temp,limit+1,limit,limit);
			else
			{
				result=i;
				printf("\n%lld is a prime factor.\n",result);
				findTheLargestPrimeFactor(num,temp/i,i,limit,result);
			}
		}
		else
			findTheLargestPrimeFactor(num,temp,i+1,limit,result);
	}
	else
		return result;
}
